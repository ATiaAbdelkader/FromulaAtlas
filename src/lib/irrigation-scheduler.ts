...
export interface Zone {
  ...
  schedules: Schedule[];
  /** Agronomic & Drought Stress attributes */
  crop?: string;                 // 'tomato', 'maize', 'bell-pepper', etc.
  fieldId?: string;              // reference to multi-field
  stage?: 'establishment' | 'vegetative' | 'flowering' | 'filling' | 'maturation';
  soilWaterMm?: number;          // current available soil water mm
  tawMm?: number;                // total available water mm
  lastDsiScore?: number;         // 0-100
  lastDsiCheck?: string;         // ISO string
}
