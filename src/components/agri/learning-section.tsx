    ...
    <section className="space-y-4">
      <div className="grid md:grid-cols-2 gap-4">
        <div className={cn('rounded-xl border-2 p-4 bg-card', color.border)}>
          <div className="flex items-center gap-2 mb-3"><div className="flex items-center justify-center h-7 w-7 rounded-md bg-gradient-to-br from-amber-400 to-orange-500 text-white"><Sparkles className="h-4 w-4" /></div><span className="text-sm font-semibold">{isRTL ? 'معادلة اليوم' : 'Formula of the Day'}</span></div>
          <button onClick={() => onOpenFormula(formulaOfTheDay)} className="group w-full text-left">
            <div className="flex items-center gap-2 mb-2"><Badge variant="outline" className={cn('font-mono text-xs font-semibold', color.bg, color.text, color.border)}>{formulaOfTheDay.code}</Badge>{hasCalculator(formulaOfTheDay.code) && <Badge variant="secondary" className="text-[10px]">{isRTL ? 'حاسبة' : 'Calculator'}</Badge>}</div>
            <h3 className="text-base font-semibold leading-tight mb-1 group-hover:text-emerald-600">{(language === 'ar' && (formulaOfTheDay as any).name_ar) ? (formulaOfTheDay as any).name_ar : formulaOfTheDay.name}</h3>
            <p className="text-xs text-muted-foreground line-clamp-2">{(language === 'ar' && (formulaOfTheDay as any).purpose_ar) ? (formulaOfTheDay as any).purpose_ar : formulaOfTheDay.purpose}</p>
            <div className="flex items-center gap-1 text-xs font-medium text-emerald-600 mt-2">{isRTL ? 'استكشف' : 'Explore'}<ArrowRight className="h-3 w-3 group-hover:translate-x-0.5 transition-transform" /></div>
          </button>
        </div>
        <div className="rounded-xl border-2 border-blue-200 dark:border-blue-900 p-4 bg-blue-50/50 dark:bg-blue-950/20">
          <div className="flex items-center gap-2 mb-3"><div className="flex items-center justify-center h-7 w-7 rounded-md bg-blue-500 text-white"><Lightbulb className="h-4 w-4" /></div><span className="text-sm font-semibold">{isRTL ? 'نصيحة اليوم' : 'Tip of the Day'}</span></div>
      ...
      <div className="rounded-xl border-2 border-border bg-card p-4">
        ...
        {nextBadges.length > 0 && (
          <div className="space-y-1.5">
            <div className="text-[10px] uppercase font-semibold text-muted-foreground">{isRTL ? 'الإنجازات القادمة' : 'Next achievements'}</div>
