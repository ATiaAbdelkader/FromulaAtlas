  ...
  return (
    <div className="mb-5 rounded-lg border border-amber-200 dark:border-amber-900 bg-amber-50/50 dark:bg-amber-950/20 p-3">
      <div className="flex items-center gap-1.5 mb-2">
        ...
        <span className="text-xs font-semibold text-amber-700 dark:text-amber-400 uppercase tracking-wide">{isRTL ? 'المعادلات المعلّمة' : 'Bookmarked Formulas'}</span>
        ...
        {bookmarkedFormulas.map(f => {
          ...
          return (
            <div key={f.code} className="group inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg border border-border bg-card hover:border-amber-400 hover:shadow-sm transition-all cursor-pointer" onClick={() => onSelect(f)}>
              ...
              <button onClick={(e) => { e.stopPropagation(); toggleBookmark(f.code); onRemove(f.code); }} className="text-amber-400 hover:text-amber-600 opacity-0 group-hover:opacity-100 transition-opacity" title={isRTL ? 'إزالة من المفضلة' : 'Remove bookmark'}>
                <X className="h-3 w-3" />
              </button>
            </div>
          );
