...
function soilConstraintCopy(language: Language, constraint: SoilConstraint): string {
  ...
  if (constraint.key === 'k') return copyFor(language, `Potassium ${constraint.value.toFixed(2)} meq/100g is low`, `البوتاسيوم ${constraint.value.toFixed(2)} ميكاف/100غ منخفض`);
  ...
  return copyFor(language, `Sodium ${constraint.value.toFixed(2)} meq/100g needs attention`, `الصوديوم ${constraint.value.toFixed(2)} ميكاف/100غ يحتاج إلى انتباه`);
}
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <StatusTile
          ...
          detail={latestSoilTest ? copyFor(language, `${formatDate(language, latestSoilTest.date)} · OM ${latestSoilTest.om.toFixed(1)}% · K ${latestSoilTest.k.toFixed(2)}`, `${formatDate(language, latestSoilTest.date)} · مادة عضوية ${latestSoilTest.om.toFixed(1)}٪ · K ${latestSoilTest.k.toFixed(2)}`) : copyFor(language, 'Add a soil test to ground nutrient decisions in current results.', 'أضف تحليل تربة لربط قرارات المغذيات بنتائج حديثة.')}
        />
        <StatusTile
          icon={ClipboardList}
