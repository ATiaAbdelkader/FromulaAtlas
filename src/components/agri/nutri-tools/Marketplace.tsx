...
import {
  ...
  TrendingDown, CheckCircle2, Truck, X, RotateCcw,
} from 'lucide-react';
import {
...
const TITLE: TrilingualString = {
  ...
  ar: 'سوق المنتجات الزراعية',
  fr: 'Marché agricole',
};

const DESC: TrilingualString = {
  en: 'Compare fertilizers, amendments, micronutrients, pesticides and seeds across suppliers — best price highlighted, cart saved locally.',
  ar: 'قارن الأسمدة والمعدّلات والعناصر الصغرى والمبيدات والبذور عبر الموردين — أفضل سعر مميّز، والسلة محفوظة محلياً.',
  fr: 'Comparez engrais, amendements, oligoéléments, pesticides et semences entre fournisseurs — meilleur prix mis en avant, panier sauvegardé localement.',
};

const PILL_LABEL: TrilingualString = { en: 'Category:', ar: 'الفئة:', fr: 'Catégorie :' };
...
export function Marketplace() {
  ...
  const productDisplay = (product: Product) => isAr ? (PRODUCT_AR[product.id] ?? { name: product.name, description: product.description, activeIngredient: product.activeIngredient }) : { name: product.name, description: product.description, activeIngredient: product.activeIngredient };

  const categoryLabel = (cat: ProductCategory | 'all') => {
    if (cat === 'all') return tr('All Products', 'كل المنتجات', 'Tous les produits');
    if (isAr) return CATEGORY_LABEL_AR[cat];
    if (isFr) return CATEGORY_LABEL_FR[cat];
  ...
  const clearCart = () => {
    ...
    saveCart([]);
    toast({ title: tr('Cart cleared', 'تم مسح السلة', 'Panier vidé') });
  };

  const total = cartTotal(cart);
  const categories: (ProductCategory | 'all')[] = ['all', 'fertilizer', 'amendment', 'micronutrient', 'pesticide', 'seed'];
  const categoryPills: CalculatorPill[] = categories.map(cat => ({ key: cat, label: categoryLabel(cat) }));

  const exportOrder = () => {
    const win = window.open('', '_blank');
    if (!win) return;
    const rows = cart.map((c, i) =>
      `<tr><td>${i + 1}</td><td>${c.emoji} ${isAr ? productDisplay(PRODUCTS.find(p => p.id === c.productId) ?? ({ id: c.productId, name: c.productName, description: '' } as Product)).name : c.productName}</td><td>${c.supplierName}</td><td style="text-align:right">${c.quantity}</td><td>${c.unit}</td><td style="text-align:right">$${c.price}</td><td style="text-align:right">$${(c.price * c.quantity).toFixed(2)}</td></tr>`
    ).join('');
    const titleText = tr('Farm Purchase Order', 'أمر شراء مزرعة', 'Bon de commande de la ferme');
    const dateText = tr('Date', 'التاريخ', 'Date');
    const itemsText = tr('items', 'عناصر', 'articles');
    const thText = isAr
    ...
    const totalText = tr('Total', 'الإجمالي', 'Total');
    win.document.write(`<!DOCTYPE html><html><head><title>${titleText}</title><style>
      ...
      <h1>🌱 ${titleText}</h1>
      <div class="meta">${dateText}: ${new Date().toLocaleDateString(isAr ? 'ar' : isFr ? 'fr-FR' : undefined)} · ${cart.length} ${itemsText}</div>
  ...
  return (
    <CalculatorShell
      ...
      description={DESC}
      accent="emerald"
      badge={cart.length > 0 ? `${cart.length} ${cart.length === 1 ? tr('item', 'عنصر', 'article') : tr('items', 'عناصر', 'articles')}` : undefined}
      actions={[
        {
          icon: RotateCcw,
          label: { en: 'Clear Cart', ar: 'مسح السلة', fr: 'Vider le panier' },
          onClick: clearCart,
        },
      ]}
      pills={categoryPills}
      activePill={activeCategory}
      onPillClick={(k) => setActiveCategory(k as ProductCategory | 'all')}
      ...
      <div className="lg:col-span-12 space-y-4">
        ...
        <div className="flex gap-2 items-center">
          <div className="relative flex-1">
            ...
            <Input value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder={tr('Search fertilizers, amendments, pesticides...', 'ابحث عن أسمدة، معدّلات، مبيدات...', 'Rechercher des engrais, amendements, produits phytosanitaires…')} className="h-9 pl-8 text-sm" />
          </div>
          <Button variant={cart.length > 0 ? 'default' : 'outline'} size="sm" onClick={() => setShowCart(!showCart)} className="gap-1.5">
            <ShoppingCart className="h-4 w-4" />
            {cart.length > 0 ? `${tr('Cart', 'السلة', 'Panier')} (${cart.length})` : tr('Cart', 'السلة', 'Panier')}
        ...
        {showCart && cart.length > 0 && (
          <div className="rounded-xl border-2 border-emerald-300 dark:border-emerald-800 bg-emerald-50/50 dark:bg-emerald-950/20 p-4 space-y-2">
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm font-bold flex items-center gap-1.5"><ShoppingCart className="h-4 w-4 text-emerald-600" /> {tr(`Shopping Cart (${cart.length})`, `سلة التسوق (${cart.length})`, `Panier (${cart.length})`)}</span>
              <div className="flex gap-2">
                <Button size="sm" variant="outline" onClick={exportOrder} className="gap-1.5 text-xs"><Download className="h-3 w-3" /> {tr('Export Order', 'تصدير الطلب', 'Exporter la commande')}</Button>
            ...
            {cart.map((item, i) => (
              <div key={i} className="flex items-center gap-2 text-xs py-1.5 border-b border-emerald-100 dark:border-emerald-900/50 last:border-0">
                ...
                <div className="flex-1 min-w-0">
                  <div className="font-medium truncate">{isAr ? productDisplay(PRODUCTS.find(p => p.id === item.productId) ?? ({ id: item.productId, name: item.productName, description: '' } as Product)).name : item.productName}</div>
            ...
            <div className="flex items-center justify-between pt-2 border-t border-emerald-200 dark:border-emerald-800">
              <span className="text-sm font-bold">{tr('Total', 'الإجمالي', 'Total')}</span>
                ...
                <div className="p-3 space-y-2">
                  ...
                  {product.applicationRate && (
                    <div className="text-[10px] bg-muted/30 rounded p-1.5">
                      <span className="font-semibold">{tr('Rate:', 'المعدل:', 'Dose :')}</span> {product.applicationRate}
                    </div>
                  )}
                      ...
                      <div key={s.name} className={`flex items-center justify-between text-xs rounded p-1.5 ${s.price === best.price ? 'bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-200 dark:border-emerald-800' : 'border border-transparent'}`}>
                        <div className="flex items-center gap-1.5 min-w-0">
                          ...
                          <div className="min-w-0">
                            ...
                            <div className="flex items-center gap-1 text-[9px] text-muted-foreground">
                              ...
                              {s.rating} · <Truck className="h-2 w-2" /> {s.deliveryDays}{tr('d', 'ي', 'j')}
                              {!s.inStock && <span className="text-red-500">{tr('Out of stock', 'غير متوفّر', 'Rupture de stock')}</span>}
                            </div>
                          </div>
                        </div>
                        <div className="flex items-center gap-2 flex-shrink-0">
                          ...
                          <Button size="sm" variant={s.price === best.price ? 'default' : 'outline'} onClick={() => addToCart(product, si)} disabled={!s.inStock}
                            className="h-6 w-6 p-0" title={tr('Add to cart', 'أضف إلى السلة', 'Ajouter au panier')}>
                  ...
                  {savings > 0 && (
                    <div className="text-[10px] text-emerald-600 dark:text-emerald-400 flex items-center gap-1">
                      ...
                      {tr(`Save ${savingsPct}% ($${savings.toFixed(2)}) by choosing best price`, `وفّر ${savingsPct}% ($${savings.toFixed(2)}) باختيار أفضل سعر`, `Économisez ${savingsPct} % ($${savings.toFixed(2)}) en choisissant le meilleur prix`)}
                    </div>
                  )}
                </div>
              </div>
        ...
        {filtered.length === 0 && (
          <div className="text-center py-8">
            ...
            <div className="text-sm text-muted-foreground">{tr('No products match your search.', 'لا توجد منتجات مطابقة لبحثك.', 'Aucun produit ne correspond à votre recherche.')}</div>
          </div>
        )}
      </div>
    </CalculatorShell>
