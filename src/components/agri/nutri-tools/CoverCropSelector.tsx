...
const localized = (language: UiLanguage, value: string, arabic: Record<string, string>) => copyFor(language, value, arabic[value] || value);
  ...
  return (
    <Card className="overflow-hidden border-lime-100 shadow-sm dark:border-lime-900/60">
      <CardHeader className="border-b border-border/60 bg-lime-50/50 pb-4 dark:bg-lime-950/10">
        <CardTitle className="flex items-center gap-2 text-base"><span className="rounded-lg bg-lime-100 p-2 text-lime-700 dark:bg-lime-900/40 dark:text-lime-300"><Sprout className="h-4 w-4" /></span> {copyFor(language, 'Cover Crop Selector', 'محدد محاصيل التغطية')}
        </CardTitle>
        <p className="text-[10px] text-muted-foreground">{copyFor(language, '12 species · 9 goals · drought tolerance · ranked recommendations', '12 نوعاً · 9 أهداف · تحمل الجفاف · توصيات مرتبة')}</p>
      </CardHeader>
      <CardContent className="space-y-3">
        <div className="rounded-xl border border-lime-200/70 bg-lime-50/30 p-3 dark:border-lime-900/60 dark:bg-lime-950/10">
          <div className="flex items-end justify-between gap-2"><div><Label className="text-xs font-semibold">{copyFor(language, 'Your goals', 'أهدافك')}</Label><p className="mt-0.5 text-xs text-muted-foreground">{copyFor(language, 'Select every outcome you want this cover crop to support.', 'حدد كل نتيجة تريد أن يدعمها محصول التغطية.')}</p></div><Badge variant="secondary" className="text-[10px]">{selectedGoals.length} {copyFor(language, 'selected', 'محددة')}</Badge></div>
            ...
            {GOALS.map(g => {
              ...
              return (
                <button type="button" key={g.id} aria-pressed={active} onClick={() => toggleGoal(g.id)} className={`min-h-10 rounded-lg border px-3 py-2 text-xs font-medium transition-colors ${active ? 'border-emerald-600 bg-emerald-600 text-white shadow-sm' : 'border-border bg-background hover:bg-muted/50'}`}>
                  {g.icon} {localized(language, g.label, GOAL_AR)}
                </button>
              );
            })}
          </div>
        ...
        <div className="grid grid-cols-1 gap-3 rounded-xl border border-border/70 bg-muted/20 p-3 sm:grid-cols-2">
          <div><div className="flex items-center justify-between gap-2"><Label className="text-xs font-medium">{copyFor(language, 'Minimum drought tolerance', 'الحد الأدنى لتحمل الجفاف')}</Label><Badge variant="outline" className="text-[10px]">{droughtTol} / 5</Badge></div><input aria-label={copyFor(language, 'Minimum drought tolerance', 'الحد الأدنى لتحمل الجفاف')} type="range" min={1} max={5} value={droughtTol} onChange={e => setDroughtTol(parseInt(e.target.value))} className="mt-3 h-2 w-full accent-emerald-600" /></div>
          <div><Label className="text-xs font-medium">{copyFor(language, 'Annual rainfall (mm)', 'الأمطار السنوية (مم)')}</Label><input aria-label={copyFor(language, 'Annual rainfall in millimeters', 'الأمطار السنوية بالملليمتر')} type="number" value={precip} onChange={e => setPrecip(e.target.value)} step="50" className="mt-1 h-10 w-full rounded-md border border-input bg-background px-3 text-sm" /></div>
        </div>

        <div className="space-y-2">
          <div><p className="text-sm font-semibold">{copyFor(language, 'Top recommendations', 'أفضل التوصيات')}</p><p className="text-xs text-muted-foreground">{copyFor(language, 'Ranked for your goals, drought threshold, and rainfall.', 'مرتبة وفق أهدافك وحد تحمل الجفاف ومعدل الأمطار.')}</p></div>
          {ranked.map((r, i) => (
            <div key={r.crop.name} className="flex items-center gap-3 rounded-xl border bg-background/70 p-3 shadow-sm" style={{ borderLeftWidth: 3, borderLeftColor: i === 0 ? '#16a34a' : '#94a3b8' }}>
              ...
              <div className="flex-1 min-w-0">
                <div className="text-sm font-semibold">{localized(language, r.crop.name, CROP_AR)}</div>
                <div className="text-xs leading-relaxed text-muted-foreground">
                  {localized(language, r.crop.type, TYPE_AR)} · {localized(language, r.crop.plantingWindow, WINDOW_AR)} · {r.crop.biomass}t {copyFor(language, 'biomass', 'كتلة حيوية')} · {r.crop.nFix > 0 ? `${r.crop.nFix} kg N/ha` : copyFor(language, 'no N fix', 'لا يثبت النيتروجين')}
                  {r.crop.winterKill && ` · ${copyFor(language, 'winter-kills', 'يموت شتاءً')}`}
                </div>
              </div>
              <Badge variant="outline" className="shrink-0 text-[10px]">{r.score.toFixed(0)} {copyFor(language, 'pts', 'نقطة')}</Badge>
