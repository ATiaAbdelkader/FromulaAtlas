    ...
    <Card>
      <CardHeader className="pb-3">
        <CardTitle className="text-base flex items-center gap-2">
          <Home className="h-4 w-4 text-emerald-600" /> Greenhouse Climate Designer
        </CardTitle>
      ...
      <CardContent className="space-y-3">
        <div className="grid grid-cols-2 gap-2">
          <div>
            <Label className="text-[10px]">Greenhouse area (m²)</Label>
            <Input value={area} onChange={e => setArea(e.target.value)} type="number" step="10" className="h-8 text-xs mt-0.5" />
          </div>
          <div>
            <Label className="text-[10px]">Wall height (m)</Label>
            <Input value={height} onChange={e => setHeight(e.target.value)} type="number" step="0.5" className="h-8 text-xs mt-0.5" />
          </div>
        </div>
        <div className="grid grid-cols-2 gap-2">
          <div>
            <Label className="text-[10px]">Glazing type</Label>
            <select value={glazing} onChange={e => setGlazing(e.target.value)} className="h-8 text-xs w-full rounded-md border border-input bg-background px-2 mt-0.5">
              {Object.entries(glazingU).map(([k, v]) => <option key={k} value={k}>{v.label} (U={v.u})</option>)}
            </select>
          </div>
          <div className="grid grid-cols-2 gap-1">
            <div>
              <Label className="text-[10px]">Inside (°C)</Label>
              <Input value={insideT} onChange={e => setInsideT(e.target.value)} type="number" className="h-8 text-xs mt-0.5" />
            </div>
            <div>
              <Label className="text-[10px]">Outside (°C)</Label>
              <Input value={outsideT} onChange={e => setOutsideT(e.target.value)} type="number" className="h-8 text-xs mt-0.5" />
            </div>
          </div>
        </div>
...
function Metric({ label, value, sub, color }: { label: string; value: string; sub?: string; color: keyof typeof ACCENT }) {
  return (
    <div className={`rounded-md border px-2 py-1.5 ${ACCENT[color]}`}>
...
function TabBtn({ active, onClick, icon: Icon, label }: { active: boolean; onClick: () => void; icon: typeof Sun; label: string }) {
  return (
    <button onClick={onClick} className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-md text-xs font-medium transition-colors ${active ? 'bg-emerald-100 dark:bg-emerald-950/50 text-emerald-700 dark:text-emerald-300' : 'text-muted-foreground hover:bg-muted/50'}`}>
