...
Generated by FormulaAtlas Soil Lab Agronomic Engine.
  ...
  return (
    <div className="w-full space-y-6 max-w-7xl mx-auto" dir={isAr ? 'rtl' : 'ltr'}>
      {/* Hidden File Input */}
      <input
        ref={fileInputRef}
        type="file"
      ...
      {/* Hero Header Banner */}
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-emerald-800 via-teal-800 to-cyan-900 text-white p-6 shadow-xl">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 relative z-10">
          <div className="space-y-1.5">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-2xl font-bold tracking-tight">
                {tr('Soil Lab CSV Analyzer & Formula Engine', 'محلل تقارير التربة المخبرية ومطابق المعادلات', 'Analyseur CSV de laboratoire de sol')}
              </h2>
            </div>
            <p className="text-sm text-emerald-100/90 max-w-3xl leading-relaxed">
              {tr(
                'Import soil test CSVs from any agricultural lab, auto-map headers (pH, CEC, Base Saturation, P, K, EC, NO₃), and compute precise fertilizer & amendment prescriptions powered by canonical Formula Atlas equations.',
                'استورد ملفات CSV لتحاليل التربة من أي مختبر زراعي، مع مطابقة آلية للأعمدة وحساب دقيق للاحتياجات السمادية والجير والجبس والكبريت اعتماداً على معادلات أطلس التسميد.',
                'Importez vos analyses de sol CSV, mappez automatiquement les colonnes et calculez vos besoins d’amendements et d’engrais selon le référentiel agronomique.'
              )}
            </p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            >
              <Upload className="h-4 w-4 mr-1 text-emerald-300" />
              {tr('Import CSV File', 'استيراد ملف CSV', 'Importer CSV')}
            </Button>
            <Button
            ...
            >
              <Download className="h-4 w-4 mr-1 text-cyan-300" />
              {tr('Export CSV', 'تصدير CSV', 'Exporter CSV')}
            </Button>
            <Button
              ...
              className="bg-white/10 hover:bg-white/20 text-white border-white/20 backdrop-blur"
            >
              {copiedPrescription ? <Check className="h-4 w-4 mr-1 text-emerald-300" /> : <Copy className="h-4 w-4 mr-1" />}
              {tr('Copy Prescription', 'نسخ الوصفة', 'Copier')}
            </Button>
            <Button
        ...
        {/* Global Settings Ribbon: Target Crop, Yield, Area */}
        <div className="mt-6 pt-5 border-t border-white/15 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 items-center">
          {/* Target Crop */}
          <div className="space-y-1">
            <Label className="text-xs text-emerald-200 font-semibold">
              {tr('Target Crop', 'المحصول المستهدف', 'Culture cible')}
            </Label>
            <Select value={selectedCropId} onValueChange={handleCropChange}>
              <SelectTrigger className="bg-black/30 border-white/20 text-white h-9 text-xs">
          ...
          <div className="space-y-1">
            ...
            <Input
              ...
              className="bg-black/30 border-white/20 text-white h-9 text-xs font-mono"
            />
          </div>
          ...
          <div className="space-y-1">
            <div className="flex items-center justify-between text-xs text-emerald-200 font-semibold">
              ...
              <span className="font-mono text-white">{fieldAreaHa} ha</span>
            </div>
            <Input
              type="number"
              min={0.1}
        ...
        <div className="mt-4 flex flex-wrap items-center gap-2 text-xs">
          <span className="text-emerald-200 font-medium">{tr('Sample Labs:', 'نماذج مختبرات جاهزة:', 'Exemples :')}</span>
          ...
          <button
            ...
            className="ml-auto px-2.5 py-1 rounded-full bg-white/20 hover:bg-white/30 border border-white/30 text-white font-medium transition-all text-[11px] flex items-center gap-1"
          >
