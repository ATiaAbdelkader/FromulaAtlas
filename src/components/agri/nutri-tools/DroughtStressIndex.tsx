...
import { Flame, AlertTriangle, CheckCircle2, Bell, Send, Check } from 'lucide-react';
...
export function DroughtStressIndex() {
  ...
  const handleSendPushAlert = async () => {
    ...
    try {
      ...
      setPushSent(true);
      toast({
        title: copyFor(language, 'Push Notification Dispatched', 'تم إرسال إشعار الدفع'),
        description: copyFor(
          language,
      ...
      toast({
        title: copyFor(language, 'Notification Error', 'خطأ في إرسال الإشعار'),
        description: copyFor(language, 'Could not deliver push notification', 'تعذر إرسال الإشعار'),
        variant: 'destructive',
      });
    } finally {
    ...
    <Card className="overflow-hidden border-orange-200/60 shadow-sm dark:border-orange-900/60">
      <CardHeader className="border-b bg-gradient-to-r from-orange-50 via-background to-amber-50/40 pb-4 dark:from-orange-950/30 dark:via-background dark:to-amber-950/20">
        <div className="flex items-center justify-between">
          ...
          <Badge variant="outline" className="text-[10px] gap-1 text-orange-700 dark:text-orange-300 border-orange-200">
            <Bell className="h-3 w-3" /> OneSignal Ready
          </Badge>
        </div>
        <p className="mt-1 text-xs leading-relaxed text-muted-foreground">{copyFor(language, 'Combines ET₀ deficit + soil water depletion + crop stage sensitivity with instant OneSignal push alerts', 'يجمع بين عجز ET₀ واستنزاف ماء التربة وحساسية مرحلة المحصول مع تنبيهات دفع OneSignal فورية')}</p>
      </CardHeader>
      <CardContent className="space-y-5 p-4 sm:p-5">
        {/* Field & Crop Context */}
        <div className="grid grid-cols-1 gap-3 rounded-xl border bg-muted/20 p-3 sm:grid-cols-2">
          <div>
            <Label className="text-xs font-medium uppercase tracking-wide text-muted-foreground">{copyFor(language, 'Field / Zone Name', 'اسم الحقل / المنطقة')}</Label>
            <Input value={fieldName} onChange={e => setFieldName(e.target.value)} placeholder="e.g. North Pivot Block A" className="mt-1 h-10 text-sm" />
          </div>
          <div>
            <Label className="text-xs font-medium uppercase tracking-wide text-muted-foreground">{copyFor(language, 'Crop Type', 'نوع المحصول')}</Label>
            <Input value={cropName} onChange={e => setCropName(e.target.value)} placeholder="e.g. Tomato / Maize" className="mt-1 h-10 text-sm" />
          </div>
        </div>
              ...
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                <div>
                  <div className="text-sm font-semibold flex items-center gap-1.5 text-cyan-900 dark:text-cyan-200">
                    ...
                    {copyFor(language, 'OneSignal Push Alert Trigger', 'إطلاق تنبيه دفع OneSignal')}
                  </div>
                  <div className="text-xs text-muted-foreground mt-0.5">
                    {result.dsiScore >= 50
                      ? copyFor(language, 'High drought stress detected! Send immediate push notification to agronomists & field crew.', 'تم اكتشاف إجهاد جفاف مرتفع! أرسل إشعار دفع فوري للمهندسين وفرق الحقل.')
                      : copyFor(language, 'Test or broadcast current field drought status to subscribed devices.', 'اختبر أو أرسل حالة جفاف الحقل الحالية للأجهزة المشتركة.')}
                  </div>
                </div>
                <Button
                ...
                >
                  {pushSent ? (
                    <>
                      ...
                      {copyFor(language, 'Alert Sent!', 'تم الإرسال!')}
                    </>
                  ) : sendingPush ? (
                    <>
                      <div className="h-3.5 w-3.5 border-2 border-white border-t-transparent rounded-full animate-spin" />
                      {copyFor(language, 'Sending Push...', 'جارٍ الإرسال...')}
                    </>
                  ) : (
                    <>
                      <Send className="h-3.5 w-3.5" />
                      {copyFor(language, 'Trigger Push Alert', 'إرسال تنبيه دفع')}
                    </>
                  )}
                </Button>
              </div>
