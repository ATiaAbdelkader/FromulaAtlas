...
import { ScenarioComparisonOverlay } from './ScenarioComparisonOverlay';
...
const STORAGE_KEY_B = 'formula-atlas-crop-simulator-b-v1';
const STORAGE_KEY_COMPARE = 'formula-atlas-crop-simulator-compare-mode-v1';
...
export function CropSimulator() {
  ...
  const [scenarioB, setScenarioB] = useState<SimulatorScenario>(() => createDefaultSimulatorScenario('potato', '2026-11-01', 3));
  ...
  const [activeVizTab, setActiveVizTab] = useState<'both' | 'water_budget' | 'growth'>('both');
  const [showPredictiveYield, setShowPredictiveYield] = useState<boolean>(true);
  ...
  useEffect(() => {
    try {
      ...
      const savedB = window.localStorage.getItem(STORAGE_KEY_B);
      if (savedB) setScenarioB(JSON.parse(savedB) as SimulatorScenario);

      const savedCompare = window.localStorage.getItem(STORAGE_KEY_COMPARE);
      if (savedCompare !== null) setCompareMode(JSON.parse(savedCompare) as boolean);
    } catch {
  ...
  const updateScenarioB = (patch: Partial<SimulatorScenario>) => setScenarioB((current) => ({ ...current, ...patch }));
  ...
  const resetB = () => {
    setScenarioB(createDefaultSimulatorScenario('potato', scenario.plantingDate, scenario.areaHa));
  };
  ...
  const toggleCompareMode = (nextValue?: boolean) => {
    const value = nextValue !== undefined ? nextValue : !compareMode;
    setCompareMode(value);
    try {
      window.localStorage.setItem(STORAGE_KEY_COMPARE, JSON.stringify(value));
    } catch {
  ...
  const saveScenario = () => {
    try {
      ...
      window.localStorage.setItem(STORAGE_KEY_B, JSON.stringify(scenarioB));
      window.localStorage.setItem(STORAGE_KEY_COMPARE, JSON.stringify(compareMode));
      setSavedNotice(true);
  ...
  const handleSelectPreset = (preset: AlgerianAgroPreset) => {
    const next = createDefaultSimulatorScenario(preset.cropId, preset.plantingDate, preset.areaHa);
    updateScenario({
      ...next,
      id: scenario.id,
      areaHa: preset.areaHa,
      plantingDate: preset.plantingDate,
      avgET0: preset.avgET0,
      irrigationSystem: preset.irrigationSystem,
      expectedYieldTPerHa: preset.expectedYieldTonsHa,
      expectedPricePerT: preset.sellingPriceDzdPerTon,
      overheadAllocationPct: scenario.overheadAllocationPct,
    });
  };
  ...
  return (
    <div dir={isRTL ? 'rtl' : 'ltr'} className="space-y-6 pb-12 print:space-y-3">
      {/* Top Banner with Toggle Button */}
          ...
          <div className="max-w-3xl">
            <div className="mb-3 flex flex-wrap items-center gap-2">
              <Badge className="border-white/20 bg-white/15 text-white">
                {tr(language, 'Algeria-aware · DZD', 'مهيّأ للجزائر · دج', 'Adapté à l’Algérie · DZD')}
              </Badge>
              <Badge className="border-white/20 bg-white/15 text-white">
                {tr(language, 'Scenario planning', 'تخطيط السيناريوهات', 'Planification par scénarios')}
              </Badge>
              {compareMode && (
                <Badge className="border-indigo-400 bg-indigo-500/30 text-indigo-200">
                  <GitCompareArrows className="mr-1.5 h-3.5 w-3.5" />
                  {tr(language, 'Comparison Mode Active', 'وضع المقارنة مفعّل', 'Mode Comparaison Actif')}
                </Badge>
              )}
            </div>
            <h2 className="text-2xl font-black tracking-tight sm:text-3xl">
              {tr(language, 'Crop Business Simulator', 'محاكي أعمال المحاصيل', 'Simulateur économique des cultures')}
            </h2>
            <p className="mt-2 max-w-2xl text-sm leading-6 text-emerald-50/90">
              {tr(
                language,
                'Turn a crop idea into a transparent field budget: calendar-driven labor, water, INPV protection choices, household overhead, yield uncertainty, and the selling price you need.',
                'حوّل فكرة المحصول إلى ميزانية حقل شفافة: يد عاملة وماء حسب التقويم، خيارات حماية من فهرس INPV، مصاريف منزلية، عدم يقين المردود، وسعر البيع المطلوب.',
                'Transformez une idée de culture en budget de parcelle transparent : main-d’œuvre et eau calées sur le calendrier, produits INPV, frais du ménage, incertitude du rendement et prix de vente nécessaire.'
              )}
            </p>
          </div>

          {/* Scenario Comparison Toggle Button */}
          <div className="flex flex-col items-start gap-3 sm:items-end">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <Button
                ...
                className={`gap-2 rounded-xl px-4 py-2.5 text-xs font-bold shadow-md transition-all ${
                  compareMode
                    ? 'border-2 border-indigo-400 bg-indigo-600 text-white hover:bg-indigo-700'
                    : 'border-2 border-white/30 bg-white/15 text-white backdrop-blur hover:bg-white/25'
                }`}
              >
                <GitCompareArrows className={`h-4 w-4 ${compareMode ? 'animate-pulse text-amber-300' : ''}`} />
                {compareMode
                  ? tr(language, 'Scenario Comparison: ON', 'مقارنة السيناريوهات: مفعّلة', 'Comparaison de scénarios : ACTIVÉE')
                  : tr(language, 'Scenario Comparison: OFF', 'مقارنة السيناريوهات: معطلة', 'Comparaison de scénarios : DÉSACTIVÉE')}
              </Button>
            </div>
            ...
            {!compareMode && (
              <div className="grid grid-cols-2 gap-2 text-xs sm:grid-cols-4">
                <div className="rounded-xl bg-white/10 p-2.5">
                  <div className="text-emerald-100">{tr(language, 'Area', 'المساحة', 'Surface')}</div>
                  <div className="mt-0.5 font-bold">{formatSimulatorNumber(scenario.areaHa, 2)} ha</div>
                </div>
                <div className="rounded-xl bg-white/10 p-2.5">
                  <div className="text-emerald-100">{tr(language, 'Crop', 'المحصول', 'Culture')}</div>
                  <div className="mt-0.5 font-bold">{selectedProfile?.emoji} {cropLabel(language, scenario.cropId, selectedProfile?.cropName ?? scenario.cropId)}</div>
                </div>
                <div className="rounded-xl bg-white/10 p-2.5">
                  <div className="text-emerald-100">{tr(language, 'Field cost', 'تكلفة الحقل', 'Coût parcelle')}</div>
                  <div className="mt-0.5 font-bold">{formatSimulatorDzd(result.totalCost)}</div>
                </div>
                <div className="rounded-xl bg-white/10 p-2.5">
                  <div className="text-emerald-100">{tr(language, 'Break-even', 'نقطة التعادل', 'Seuil')}</div>
                  <div className="mt-0.5 font-bold">{formatSimulatorDzd(result.breakEvenPricePerT)}/t</div>
                </div>
              </div>
            )}
          </div>
      ...
      {/* RENDER COMPARISON OVERLAY OR SINGLE SCENARIO STEPS */}
      {compareMode ? (
        <ScenarioComparisonOverlay
          scenarioA={scenario}
          scenarioB={scenarioB}
          onUpdateScenarioA={updateScenario}
          onUpdateScenarioB={updateScenarioB}
          onCloneAtoB={cloneAtoB}
          onSwapScenarios={swapScenarios}
          onResetB={resetB}
          phytoIndex={phytoIndex}
        />
      ) : (
        <>
          {/* Quick invite banner to comparison mode */}
          <div className="flex items-center justify-between rounded-xl border border-indigo-200 bg-gradient-to-r from-indigo-50 to-emerald-50 p-3.5 text-xs text-indigo-950 dark:border-indigo-900 dark:from-indigo-950/40 dark:to-emerald-950/40 dark:text-indigo-200">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <strong className="font-bold text-foreground">
                  {tr(language, 'Want to compare two crops or irrigation systems side-by-side?', 'هل ترغب في مقارنة محصولين أو نظامي ري جنباً إلى جنب؟', 'Voulez-vous comparer deux cultures ou systèmes d’irrigation côte à côte ?')}
                </strong>
                <p className="text-muted-foreground">
                  {tr(language, 'Toggle Scenario Comparison to analyze margins, water productivity, and risk resilience simultaneously.', 'فعّل وضع مقارنة السيناريوهات لتحليل الهوامش وإنتاجية المياه ومقاومة المخاطر في وقت واحد.', 'Activez le mode comparaison pour analyser marges, valorisation de l’eau et résilience aux risques.')}
                </p>
              </div>
            </div>
            <Button
              size="sm"
              variant="outline"
              className="gap-1.5 border-indigo-300 bg-white text-indigo-800 hover:bg-indigo-50 dark:bg-slate-900 dark:text-indigo-200"
              onClick={() => toggleCompareMode(true)}
            >
              <GitCompareArrows className="h-3.5 w-3.5" />
              {tr(language, 'Open Comparison', 'فتح المقارنة', 'Ouvrir comparaison')}
            </Button>
          </div>

          {/* 1-Click Algerian Agro-Climatic Regional Presets */}
          <RegionalPresetSelector
            onSelectPreset={handleSelectPreset}
            currentCropId={scenario.cropId}
          />

          {/* Expert Agronomic Verdict & Action Advisory Banner */}
          <AgronomicVerdictBanner
            cropName={cropLabel(language, scenario.cropId, selectedProfile?.cropName ?? scenario.cropId)}
            cropEmoji={selectedProfile?.emoji ?? '🌱'}
            areaHa={scenario.areaHa}
            grossMarginDzd={result.grossMargin}
            totalWaterM3={result.totalSeasonIrrigationM3}
            irrigationSystem={scenario.irrigationSystem}
            avgET0={scenario.avgET0}
          />
          ...
          {/* Agronomic Visualizer Center: Growth Stages & Water Budget Charts */}
          <div className="space-y-4">
            <div className="flex flex-col justify-between gap-3 rounded-2xl border border-border bg-card p-3 sm:flex-row sm:items-center">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <span className="text-xs font-bold text-foreground">
                    {tr(language, 'Agronomic Lifecycle Visualizers (Recharts)', 'المخططات الزراعية البيانية (Recharts)', 'Visualiseurs agronomiques (Recharts)')}
                  </span>
                  <p className="text-[11px] text-muted-foreground">
                    {tr(language, 'Compare crop growth dynamics, canopy cover, and FAO-56 net irrigation water budget.', 'قارن ديناميكيات نمو المحصول، الغطاء الخضري، وميزانية مياه الري وفق FAO-56.', 'Analysez la croissance, le couvert et le bilan hydrique d’irrigation nette FAO-56.')}
                  </p>
                </div>
              </div>
              ...
              <div className="inline-flex rounded-xl border border-border bg-muted/60 p-1 text-xs">
                ...
                <button
                  type="button"
                  onClick={() => setActiveVizTab('growth')}
                  className={`flex items-center gap-1.5 rounded-lg px-3 py-1.5 font-semibold transition-all ${
                    activeVizTab === 'growth'
                ...
                >
                  ...
                  <span>{tr(language, 'Growth & Phenology', 'مراحل النمو والغطاء', 'Croissance & Canopée')}</span>
                </button>
                <button
                  type="button"
                  onClick={() => setActiveVizTab('both')}
                  className={`flex items-center gap-1.5 rounded-lg px-3 py-1.5 font-semibold transition-all ${
                    activeVizTab === 'both'
                ...
                >
                  ...
                  <span>{tr(language, 'Both Charts', 'كلا المخططين', 'Les deux graphiques')}</span>
                </button>
              </div>
            </div>

            {/* Visualizer 1: Water Budget (Net Irrigation vs Cumulative ETc) */}
            {(activeVizTab === 'water_budget' || activeVizTab === 'both') && (
              <CropWaterBudgetVisualizer
                ...
                irrigationSystem={scenario.irrigationSystem}
                waterCostPerM3={
                  scenario.costs.find((c) => c.category === 'irrigation')?.amount
                    ? Math.max(1, Math.round((scenario.costs.find((c) => c.category === 'irrigation')?.amount ?? 18000) / Math.max(1, (result.totalSeasonIrrigationM3 / Math.max(0.1, scenario.areaHa)))))
                    : 18
                }
              />
            )}

            {/* Visualizer 2: Real-time / Projected Crop Growth Stages & Canopy Cover */}
            {(activeVizTab === 'growth' || activeVizTab === 'both') && (
          ...
          <div className="rounded-2xl border border-border bg-card p-4 shadow-sm sm:p-6">
            <SectionHeading step={tr(language, 'Step 4', 'الخطوة ٤', 'Étape 4')} icon={TrendingUp} title={tr(language, 'Set yield and market assumptions', 'حدّد المردود وافتراضات السوق', 'Définir le rendement et le marché')} description={tr(language, 'Use the predictive agronomic engine with Algeria historical baselines, or manually customize your market expectations.', 'استخدم المحرك الزراعي التنبؤي المعتمد على المراجع التاريخية الجزائرية، أو أدخل توقعات السوق يدويًا.', 'Utilisez le moteur agronomique prédictif basé sur les historiques algériens, ou personnalisez vos attentes de marché.')} />
            
            {/* Predictive Yield Engine Box */}
            <div className="mt-5">
              <PredictiveYieldCalculator
                ...
                currentIrrigationSystem={scenario.irrigationSystem}
                onApplyPredictedYield={(predictedYield) => {
                  updateScenario({ expectedYieldTPerHa: predictedYield });
                }}
              />
            </div>
      ...
      {/* Footer Controls */}
      <div className="flex flex-col gap-3 border-t border-border pt-4 sm:flex-row sm:items-center sm:justify-between print:hidden">
        <div className="flex flex-wrap gap-2">
          <Button variant="outline" className="gap-2" onClick={saveScenario}>
            <CheckCircle2 className="h-4 w-4" />
            {savedNotice ? tr(language, 'Saved', 'تم الحفظ', 'Enregistré') : tr(language, 'Save scenario', 'حفظ السيناريو', 'Enregistrer le scénario')}
          </Button>
          <Button variant="outline" className="gap-2" onClick={resetScenario}>
            <RefreshCw className="h-4 w-4" />
            {tr(language, 'Reset', 'إعادة تعيين', 'Réinitialiser')}
          </Button>
        </div>
        <Button className="gap-2 bg-emerald-700 hover:bg-emerald-800" onClick={() => window.print()}>
          <Printer className="h-4 w-4" />
          {tr(language, 'Print decision brief', 'طباعة ملخص القرار', 'Imprimer le brief de décision')}
        </Button>
      </div>
      ...
      <p className="text-center text-[11px] text-muted-foreground print:hidden">
        {tr(
          language,
          'Planning support only. Verify current OAIC prices, supplier quotes, INPV registrations, labels, labor rates, and water/energy tariffs before making a commitment.',
          'أداة دعم للتخطيط فقط. تحقّق من أسعار OAIC الحالية وعروض الموردين وتسجيلات INPV والملصقات ومعدلات العمل وتعريفات الماء والطاقة قبل الالتزام.',
          'Outil d’aide à la planification. Vérifiez les prix OAIC actuels, les devis fournisseurs, les homologations INPV, les étiquettes, les salaires et les tarifs d’eau/énergie avant tout engagement.'
        )}
      </p>
    </div>
  );
