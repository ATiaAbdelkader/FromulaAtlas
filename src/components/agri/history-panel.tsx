...
function formatRelative(timestamp: number, isRTL: boolean): string {
  const diff = Date.now() - timestamp;
  const mins = Math.floor(diff / 60000); const hours = Math.floor(mins / 60); const days = Math.floor(hours / 24);
  if (isRTL) {
    if (mins < 60) return `منذ ${mins} د`; if (hours < 24) return `منذ ${hours} س`; if (days < 7) return `منذ ${days} يوم`;
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="right" className="p-0 gap-0 w-full sm:w-[440px] overflow-hidden flex flex-col">
        <SheetHeader className="p-4 pb-3 border-b border-border bg-gradient-to-r from-emerald-50 to-background dark:from-emerald-950/30 dark:to-background space-y-1">
          <SheetTitle className="flex items-center gap-2 text-base"><History className="h-4 w-4 text-emerald-600" /> {isRTL ? 'سجل الحسابات' : 'Calculation History'}</SheetTitle>
          <SheetDescription className="flex items-center justify-between">
            <span>{calcHistory.length === 0 ? (isRTL ? 'ستظهر حساباتك هنا.' : 'Your calculations will appear here.') : isRTL ? `${calcHistory.length} حساب محفوظ` : `${calcHistory.length} saved calculation${calcHistory.length === 1 ? '' : 's'}`}</span>
            {calcHistory.length > 0 && <Button variant="ghost" size="sm" onClick={clearHistory} className="h-7 text-xs gap-1 text-muted-foreground hover:text-destructive"><Trash2 className="h-3 w-3" />{isRTL ? 'مسح الكل' : 'Clear all'}</Button>}
          </SheetDescription>
        </SheetHeader>
        {calcHistory.length === 0 ? (
          <div className="flex-1 flex flex-col items-center justify-center p-8 text-center">
            <div className="rounded-full bg-muted p-4 mb-3"><History className="h-8 w-8 text-muted-foreground" /></div>
            <h3 className="text-base font-semibold mb-1">{isRTL ? 'لا توجد حسابات بعد' : 'No calculations yet'}</h3>
            <p className="text-sm text-muted-foreground max-w-xs">{isRTL ? 'افتح أي معادلة بحاسبة وشغّل حساباً. سيُحفظ هنا تلقائياً.' : 'Open any formula with a calculator and run a calculation. It will be saved here automatically.'}</p>
          </div>
        ) : (
          <ScrollArea className="flex-1 min-h-0">
            <div className="p-3 space-y-2">
              {calcHistory.map(entry => {
                ...
                const formulaName = (language === 'ar' && formula && (formula as any).name_ar) ? (formula as any).name_ar : entry.formulaName;
                return (
                  <div key={entry.id} className="group rounded-lg border border-border bg-card p-3 hover:border-emerald-300 dark:hover:border-emerald-800 transition-colors">
                    <div className="flex items-start justify-between gap-2 mb-2">
                      <div className="flex items-center gap-2 min-w-0 flex-1">
                        ...
                        <button onClick={() => { if (onSelectFormula) { onOpenChange(false); onSelectFormula(entry.formulaCode); } }} className="text-sm font-medium hover:text-emerald-600 dark:hover:text-emerald-400 transition-colors text-left truncate" title={isRTL ? 'افتح المعادلة' : 'Open formula'}>{formulaName}</button>
                      </div>
                      <button onClick={() => removeEntry(entry.id)} className="opacity-0 group-hover:opacity-100 transition-opacity text-muted-foreground hover:text-destructive flex-shrink-0" title={isRTL ? 'إزالة' : 'Remove'}><Trash2 className="h-3.5 w-3.5" /></button>
                    </div>
                    ...
                    <div className="flex items-center gap-2 rounded-md bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-200 dark:border-emerald-900 px-2.5 py-1.5">
                      ...
                      <span className="ml-auto text-[10px] text-muted-foreground whitespace-nowrap">{formatRelative(entry.timestamp, isRTL)}</span>
                    </div>
