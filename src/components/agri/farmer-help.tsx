...
export function FarmerHelp({ onOpenTool }: FarmerHelpProps) {
  ...
  const faqs = useMemo(() => [
    ...
    { icon: Bug, color: '#e11d48', q: tr('What pest is eating my plants?', 'ما الآفة التي تأكل نباتاتي؟', 'Quel ravageur mange mes plantes ?'), action: () => onOpenTool('farm', 'collapse_ipm_action') },
    ...
    { icon: Sprout, color: '#16a34a', q: tr('What crop should I plant next?', 'ما المحصول التالي؟', 'Quelle culture planter ensuite ?'), action: () => onOpenTool('farm', 'collapse_rotation') },
  ], [language]);
