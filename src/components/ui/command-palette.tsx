...
import {
  searchTools, recordToolUse, getPinnedToolIds, toggleToolPin, localizeToolEntry,
...
interface CommandPaletteProps {
  ...
  /** Active experience; registry results are filtered to this level. */
  level: UserLevel;
  /** Called when the user selects a tool. Parent switches tab + opens section. */
  onSelect: (entry: ToolEntry) => void;
}

export function CommandPalette({ open, onOpenChange, level, onSelect }: CommandPaletteProps) {
  ...
  const results = useMemo(() => searchTools(query, 20, level), [level, query]);
  ...
  const visiblePinnedCount = useMemo(() => results.filter(result => pinnedIds.includes(result.entry.id)).length, [pinnedIds, results]);
      ...
      <DialogContent dir={isRTL ? 'rtl' : 'ltr'} className="p-0 overflow-hidden max-w-2xl gap-0" aria-describedby={undefined}>
        ...
        <Command className="rounded-lg" shouldFilter={false}>
          ...
          <div className="flex items-center gap-1 border-b px-3 py-2">
            ...
            >
              <Pin className="h-3 w-3" /> {language === 'ar' ? 'المثبّتة' : language === 'fr' ? 'Épinglés' : 'Pinned'} <span className="font-mono">{visiblePinnedCount}</span>
            </button>
          </div>
          ...
          <div className="border-t flex items-center justify-between px-3 py-2 text-[10px] text-muted-foreground">
            ...
            <span className="flex items-center gap-1">
              <CornerDownLeft className="h-3 w-3" /> {results.length} {language === 'ar' ? 'أداة متاحة' : language === 'fr' ? 'outils disponibles' : 'tools available'}
            </span>
          </div>
        </Command>
      </DialogContent>
